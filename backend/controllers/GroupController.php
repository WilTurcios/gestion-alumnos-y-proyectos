...
{
  public function __construct(private $groupService)
  {
  }
  ...
  public function deleteManyGroups(?array $grupo_ids): Response
  {
    if (is_null($grupo_ids)) {
      return new Response(
        false,
        400,
        'Bad Request: Asegúrate de proporcionar los datos necesarios para la eliminación de los grupos'
      );
    }
    ...
    if (!is_array($grupo_ids)) {
      return new Response(
        false,
        400,
        'Bad Request: Asegúrate de proporcionar un array de IDs para eliminar los grupos'
      );
    }
    ...
    $result = $this->groupService->deleteMany($grupo_ids);

    if (!$result) {
      return new Response(
        false,
        500,
        'Ha ocurrido un error al eliminar los grupos, por favor inténtelo de nuevo'
      );
    }

    return new Response(true, 204, 'Los grupos han sido eliminados correctamente');
  }
